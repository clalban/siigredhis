...
function municipios_insert(&$error_message = '') {
	...
	$data = [
		'VcrIdExp' => Request::val('VcrIdExp', ''),
		'VcrExpEn' => Request::val('VcrExpEn', ''),
	];

	if($data['VcrIdExp'] === '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">{$Translation['error:']} 'CODIGO CIUDAD': {$Translation['field not null']}<br><br>";
	...
	if(function_exists('municipios_after_insert')) {
		$res = sql("SELECT * FROM `municipios` WHERE `VcrIdExp`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
...
function municipios_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	sql("DELETE FROM `municipios` WHERE `VcrIdExp`='{$selected_id}'", $eo);
...
function municipios_update(&$selected_id, &$error_message = '') {
	...
	if(function_exists('municipios_after_update')) {
		$res = sql("SELECT * FROM `municipios` WHERE `VcrIdExp`='{$data['selectedID']}' LIMIT 1", $eo);
	...
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "', `pkValue`='{$data['VcrIdExp']}' WHERE `tableName`='municipios' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
...
function municipios_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		$res = sql("SELECT * FROM `municipios` WHERE `VcrIdExp`='" . makeSafe($selected_id) . "'", $eo);
	...
	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'C details', $templateCode);
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		$jsReadOnly = '';
		$jsReadOnly .= "\tjQuery('#VcrIdExp').replaceWith('<div class=\"form-control-static\" id=\"VcrIdExp\">' + (jQuery('#VcrIdExp').val() || '') + '</div>');\n";
		$jsReadOnly .= "\tjQuery('#VcrExpEn').replaceWith('<div class=\"form-control-static\" id=\"VcrExpEn\">' + (jQuery('#VcrExpEn').val() || '') + '</div>');\n";
	...
	// process images
	$templateCode = str_replace('<%%UPLOADFILE(VcrIdExp)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(VcrExpEn)%%>', '', $templateCode);

	// process values
	if($selected_id) {
		if( $dvprint) $templateCode = str_replace('<%%VALUE(VcrIdExp)%%>', safe_html($urow['VcrIdExp']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(VcrIdExp)%%>', html_attr($row['VcrIdExp']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(VcrIdExp)%%>', urlencode($urow['VcrIdExp']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(VcrExpEn)%%>', safe_html($urow['VcrExpEn']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(VcrExpEn)%%>', html_attr($row['VcrExpEn']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(VcrExpEn)%%>', urlencode($urow['VcrExpEn']), $templateCode);
	} else {
		$templateCode = str_replace('<%%VALUE(VcrIdExp)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(VcrIdExp)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(VcrExpEn)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(VcrExpEn)%%>', urlencode(''), $templateCode);
	}
